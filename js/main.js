...
(function () {
  ...
  const tiltCards = document.querySelectorAll(
    '.grid-3 > .hud, .grid-4 > .hud, .cat-chip, .card, .prod-card, .sched-card, .t-division-card, .t-perk-card'
  );
  tiltCards.forEach(function (card) {
    ...
    card.addEventListener('pointermove', function (e) {
      ...
      card.style.transform = 'perspective(900px) rotateX(' + (-py * 5).toFixed(2) + 'deg) rotateY(' + (px * 6.5).toFixed(2) + 'deg) translateY(-6px)';
    });
    card.addEventListener('pointerleave', function () {
      card.style.transition = 'transform .42s var(--ease-spring)';
      card.style.transform = '';
      setTimeout(function () { card.style.transition = ''; }, 440);
    });
  });
})();
